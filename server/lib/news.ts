...
export async function generateDailyNews(): Promise<DailyNews | null> {
  ...
  const prompt = `أنت كاتب محتوى تعليمي متخصص في تاريخ الاختراعات للطلاب. مهمتك كتابة قصة مذهلة وموثوقة عن اختراع بناءً على وقائع تاريخية مثبتة فقط.

الاختراع: ${contextLine}

⚠️ قواعد صارمة للمصداقية والتوثيق:
- استخدم فقط المعلومات الموثقة تاريخياً بشكل قاطع: السنة الصحيحة، الاسم الحقيقي للمخترع، الحقيقة المثبتة
- إذا لم تكن متأكداً 100٪ من تاريخ أو اسم أو رقم — لا تكتبه أبداً واستخدم معلومة عامة مثبتة بدلاً منه
- لا تخترع حقائق أو تكمّل ما لا تعرفه — الدقة أهم من الإثارة
- أمثلة على المعلومات الموثقة المقبولة:
  * "اخْتَرَعَ غراهام بيل الهاتف عام ١٨٧٦" ✓ (تاريخ مثبت)
  * "صنع إديسون أكثر من ألف براءة اختراع في حياته" ✓ (رقم مثبت)
  * "أُطلق سبوتنيك عام ١٩٥٧ من الاتحاد السوفيتي" ✓ (حدث موثق)
- أمثلة على ما هو ممنوع:
  * نسب قول لم يُثبَت تاريخياً للمخترع ✗
  * اختراع تفصيل درامي لتجميل القصة ✗
  * خلط تواريخ أو نسب اختراع لشخص خطأ ✗
...
1. اكتب فقرة قصيرة ومثيرة بالعربية الفصحى مع الحركات الكاملة (التشكيل) عن هذا الاختراع.
   ...
   - يجب أن تتضمن: سنة الاختراع الصحيحة، اسم المخترع الحقيقي، حقيقة مفاجئة موثقة
   - الأسلوب: قصصي مثير، يثير الدهشة، يجعل الطالب يرغب في مشاركته مع أصدقائه
   - ⚠️ ممنوع تماماً: سياسة، دين طائفي، رياضة، معلومات غير موثقة
